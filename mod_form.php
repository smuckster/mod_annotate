...
class mod_annotate_mod_form extends moodleform_mod {
    ...
    public function data_preprocessing(&$defaultvalues) {
        if ($this->current->instance) {
            $format = $defaultvalues['documentformat'];
            $text = file_prepare_draft_area($draftitemid, $this->context->id, 'mod_annotate',
                'document', 0, annotate_get_editor_options($this->context), $defaultvalues['document']);

            $defaultvalues['document'] = ['format' => $format, 'text' => $text];
        }
    }
}
